...
**Implemented and locally tested; live Meta acceptance remains unverified.** No deployment, production order, real payment, OTP, WhatsApp message or live Meta event was generated. The browser inventory was empty and no Test Events code is configured. Automated tests are not proof of receipt in Events Manager.
...
| InitiateCheckout | Once/document, valid quote/commerce fields | Preserve trigger; shared UUID, CAPI |
| Purchase | Response-confirmed; authorization accepted; potentially stale cart; no guard | Capture gate, order snapshot, deterministic ID, durable CAPI and browser guard |
...
| valour_purchase | Custom successful-order metric | Preserve browser-only; separate stable ID/guard |
...
| valour_begin_checkout | Validated Place Order attempt | Preserve; shared UUID/CAPI; never standard Purchase |
| valour_payment_select | Selection change | Preserve; shared UUID/CAPI |
...
| valour_checkout_step_review | Review entry, current total | Preserve; shared UUID/CAPI |
...
Browser observations get one UUID, passed to Pixel and POST /api/meta/events. The endpoint allows only 18 named non-Purchase events, requires the configured site origin, rejects cross-site requests, validates timestamps/IDs/size/commerce fields and limits each IP to 120 requests/minute/process. It constructs its own payload, never accepting tokens, Graph URLs or arbitrary user_data. Origin checks and rate limits are abuse mitigation, not human authentication.

Both Purchase names are rejected by that endpoint. Standard Purchase comes only from persisted order state. One module sends to the fixed Graph host using bearer authorization, a five-second timeout, safe logs and nonthrowing transport failures.
...
| InitiateCheckout | First valid nonempty checkout quote | Browser observation with server quote | Shared UUID, once/document | INR/quote total, content_type, content_ids, contents, summed num_items |
| Purchase | COD response or captured online verification response | Persisted website order, confirmed COD or capture evidence, pricingSnapshot | purchase_<MongoDB ID>; persistent server state/browser guard | INR/final total, content_type, order_id, content_ids, contents, summed num_items |

Late capture after authorization produces server-only standard Purchase. A new order gets a different ID. Reloading checkout may be a new checkout observation, never a new Purchase.
...
valour_purchase remains browser-only with valour_purchase_<order ID>. Its original successful-order meaning, including authorization, is preserved; standard Purchase separately waits for capture. It does not substitute for or generate standard Purchase. Meta contact/error filtering does not strip existing Google/dataLayer payloads.

## 6. Event IDs

Non-Purchase uses crypto.randomUUID once per invocation: Pixel eventID equals CAPI event_id. Purchase uses purchase_<real internal order ID>. Retry preserves ID/time. The existing checkout customer-event ID is untouched.

Memory and localStorage markers suppress repeated browser Purchase calls. If storage fails, memory still works within the document and IDs stay deterministic. The success page has no Purchase trigger. Meta merges corresponding name/ID pairs; exactly-once networking is not claimed.

## 7. Purchase correctness and reliability

- COD counts the server-confirmed placed order, not cash collection. Later cancellation/refund adjustment is not implemented.
- Online checkout decisions remain unchanged. Authorized-only saved orders have requiresCapture=true, no browser standard Purchase. The signed payment.captured webhook records metaCapturedAt; the worker waits for this evidence. A webhook arriving before completed-order insertion is discoverable afterward.
- Captured payment with no completed order is not converted into a fabricated order/Purchase. The existing browser-verification dependency needs separate order-system work.
...
| META_CAPI_TEST_EVENT_CODE | Optional; included only in explicit development/test |
...
The existing META_CAPI_TOKEN is supported now. To adopt the canonical name, copy its value privately within the host's secret settings to META_CAPI_ACCESS_TOKEN; no source edit or token disclosure is needed. Configure PUBLIC_SITE_URL including the correct www choice; redirect aliases consistently. Trust only actual proxy subnets, never blanket forwarded headers.
...
| Check | Result |
|---|---|
| node --test backend/meta.test.js | PASS: 8 tests, multiple scenarios/test |
...
| Browser behavior | VM tests for shared IDs, initialization, privacy, refresh and separate orders; no real-browser full funnel |
...
2. Open **Events Manager → Data Sources → dataset 2927690960901306 → Test Events**. Confirm the ID. Privately set its server test code as staging META_CAPI_TEST_EVENT_CODE with NODE_ENV=test and restart. Set PUBLIC_SITE_URL to staging's origin.
...
9. Exercise all custom events through actual sandbox actions: valid/invalid coupon, OTP send/verify, quantity/removal, cart/review/progress clicks, payment selection/submission/failure, unavailable delivery area. Preserve intentional repeated actions. valour_purchase is browser-only, never another standard Purchase.
