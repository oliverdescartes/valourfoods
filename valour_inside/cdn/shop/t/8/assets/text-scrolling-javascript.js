window.addEventListener("load", () => {
    gsap.registerPlugin(ScrollTrigger), setTimeout(initTextScrollAnimation, 3e3);
}),
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        (!loadingPhase || loadingPhase.style.display === "none") &&
            (gsap.registerPlugin(ScrollTrigger), setTimeout(initTextScrollAnimation, 500));
    });
function initTextScrollAnimation() {
    ...
    function wrapWordsInSpans(element) {
        const walker = document.createTreeWalker(
            ...
            textNodes = [];
        let node;
        for (; (node = walker.nextNode()); ) node.textContent.trim() && textNodes.push(node);
    ...
    blushContainer
        ? (console.log("Blush container found and will be animated"), blushContainer.classList.remove("active"))
        : console.warn("Blush container not found - animations may not work correctly");
    const masterTimeline = gsap.timeline({
            scrollTrigger: { trigger: textRevealContainer, start: "top 70%", end: "top 50%", scrub: 2, markers: !1 },
        }),
